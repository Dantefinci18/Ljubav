...
function Producto({tag, backLink}) {
  ...
  useEffect(() => {
    ...
    if (meta) {
        meta.setAttribute("content", producto.metaDescripcion);
    }

    return () => {
        document.title = "Ljubav | Pastelería Artesanal en Núñez, Tortas y Regalos";

        if (meta) {
            meta.setAttribute(
                "content",
                "Ljubav — Pastelería artesanal de autor en Núñez. Pedidos con 48hs de anticipación y retiro en local."
            );
        }
    };
}, [producto]);
